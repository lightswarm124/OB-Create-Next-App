...
contract TokenRegistry is Ownable {
	...
	modifier noIPFSRegistered(bytes32 _IPFSVerHash) {
		require(tokenDataHash[_IPFSVerHash] == address(0));
		_;
	}
	...
	function addToken(address _token, bytes32 _dataVerHash, bytes32 _IPFSHash) public
		notYetRegistered(_token)
		noNameRegistered(_dataVerHash)
		noIPFSRegistered(_IPFSHash)
	...
	{
		require(
			tokenAddressList[_index] == _token
			&& msg.sender == tokens[_token].deployer
		);
